...
const PRODUCTOS_MAESTROS_DB = path.join(DATA_DIR_PRODUCTOS, "productos_maestros.json");
...
app.post("/api/upload-productos", uploadProductos.any(), async (req, res) => {
    try {
        ...
        if (nuevosProductos.length > 0) {
            ...
            // NUEVO: Guardar también en la base de datos maestra
            let productosMaestros = readJSON(PRODUCTOS_MAESTROS_DB);
            
            for (const producto of nuevosProductos) {
                // Buscar si ya existe en productos maestros por SKU
                const indiceMaestro = productosMaestros.findIndex(pm => pm.codSC === producto.codSC);
                ...
                const productoMaestro = {
                    ...
                    fechaCreacion: indiceMaestro >= 0 ? productosMaestros[indiceMaestro].fechaCreacion : new Date().toISOString(),
                    fechaModificacion: new Date().toISOString()
                };
                
                if (indiceMaestro >= 0) {
                    // Actualizar producto maestro existente (solo si tiene más información)
                    productosMaestros[indiceMaestro] = productoMaestro;
                } else {
                    // Agregar nuevo producto maestro
                    productosMaestros.push(productoMaestro);
                }
            }
            
            writeJSON(PRODUCTOS_MAESTROS_DB, productosMaestros);
...
// ============================================================
//  SECCIÓN B.1: PRODUCTOS MAESTROS (BASE DE DATOS CENTRALIZADA)
// ============================================================

// 1. BUSCAR PRODUCTO MAESTRO POR SKU
app.get("/api/buscar-producto-maestro", (req, res) => {
    try {
        const { sku } = req.query;
        ...
        const productosMaestros = readJSON(PRODUCTOS_MAESTROS_DB);
        const productoEncontrado = productosMaestros.find(p => p.codSC === sku);
        
        if (productoEncontrado) {
            res.json({ ok: true, encontrado: true, producto: productoEncontrado });
        } else {
            res.json({ ok: true, encontrado: false });
        }

    } catch (error) {
        console.error("Error buscar-producto-maestro:", error);
        res.status(500).json({ error: 'Error interno' });
    }
});

// 2. OBTENER TODOS LOS PRODUCTOS MAESTROS
app.get("/api/productos-maestros", (req, res) => {
    try {
        const productosMaestros = readJSON(PRODUCTOS_MAESTROS_DB);
        res.json({ ok: true, productos: productosMaestros });
    } catch (error) {
        console.error("Error obtener productos-maestros:", error);
        res.status(500).json({ error: 'Error interno' });
    }
});

// 3. BUSCAR SUGERENCIAS DE SKU (AUTOCOMPLETADO)
app.get("/api/sugerencias-sku", (req, res) => {
    try {
        ...
        // Buscar coincidencias en SKU, nombre de repuesto y marca
        const sugerencias = productosMaestros
            .filter(p => 
                p.codSC.toLowerCase().includes(queryLower) ||
                p.repuesto.toLowerCase().includes(queryLower) ||
        ...
        console.error("Error sugerencias-sku:", error);
        res.status(500).json({ error: 'Error interno' });
    }
});

// 4. AGREGAR/ACTUALIZAR PRODUCTO MAESTRO
app.post("/api/guardar-producto-maestro", uploadProductos.any(), async (req, res) => {
    try {
        ...
        // Buscar si ya existe un producto con este SKU
        const indiceExistente = productosMaestros.findIndex(p => p.codSC === producto.codSC);
        
        // Procesar imágenes si hay
        const imagenes = files.map(f => `/uploads/productos/${f.filename}`);
        ...
        const productoMaestro = {
            ...
            referenciaCruzada: producto.referenciaCruzada || '',
            oem: producto.oem || '',
            imagenes: imagenes.length > 0 ? imagenes : (producto.imagenes || []),
            fechaCreacion: indiceExistente >= 0 ? productosMaestros[indiceExistente].fechaCreacion : new Date().toISOString(),
            fechaModificacion: new Date().toISOString()
        };
        ...
        writeJSON(PRODUCTOS_MAESTROS_DB, productosMaestros);
        res.json({ ok: true, message: 'Producto maestro guardado' });

    } catch (error) {
        console.error("Error guardar-producto-maestro:", error);
        res.status(500).json({ error: 'Error interno' });
    }
});
...
// Endpoint PUT para actualizar productos maestros desde el admin
app.put("/api/actualizar-producto-maestro", (req, res) => {
    try {
        const { skuOriginal, productoEditado } = req.body;
        
        if (!skuOriginal || !productoEditado) {
            return res.status(400).json({ ok: false, msg: 'Datos incompletos' });
        }

        let productosMaestros = readJSON(PRODUCTOS_MAESTROS_DB);
        
        // Buscar el producto por SKU original
        const indice = productosMaestros.findIndex(p => p.codSC === skuOriginal);
        
        if (indice === -1) {
            return res.json({ ok: false, msg: 'Producto no encontrado' });
        }

        // Si se cambió el SKU, verificar que no exista otro producto con el nuevo SKU
        if (productoEditado.codSC !== skuOriginal) {
            const existeNuevoSKU = productosMaestros.some((p, idx) => 
                p.codSC === productoEditado.codSC && idx !== indice
            );
            
            if (existeNuevoSKU) {
                return res.json({ ok: false, msg: 'Ya existe otro producto con ese SKU' });
            }
        }

        // Actualizar el producto manteniendo fechas y otros campos
        productosMaestros[indice] = {
            ...productosMaestros[indice],
            codSC: productoEditado.codSC || productosMaestros[indice].codSC,
            repuesto: productoEditado.repuesto || productosMaestros[indice].repuesto,
            marca: productoEditado.marca || productosMaestros[indice].marca,
            linea: productoEditado.linea || productosMaestros[indice].linea,
            codCliente: productoEditado.codCliente || productosMaestros[indice].codCliente,
            stock: productoEditado.stock !== undefined ? productoEditado.stock : productosMaestros[indice].stock,
            fichaTecnica: productoEditado.fichaTecnica || productosMaestros[indice].fichaTecnica,
            referenciaCruzada: productoEditado.referenciaCruzada || productosMaestros[indice].referenciaCruzada,
            oem: productoEditado.oem || productosMaestros[indice].oem,
            fechaModificacion: new Date().toISOString()
        };
        ...
        console.error("Error actualizar-producto-maestro:", error);
        res.status(500).json({ ok: false, msg: 'Error interno al actualizar producto' });
    }
});

// Eliminar producto maestro y sus referencias en productos_db.json
app.delete("/api/eliminar-producto-maestro", (req, res) => {
    try {
        const { sku, adminKey } = req.body || {};
        if (!sku) {
            return res.status(400).json({ ok: false, msg: 'SKU requerido' });
        }
        // Validación simple de clave admin según requerimiento del frontend
        if (adminKey !== 'star4321') {
            return res.status(403).json({ ok: false, msg: 'Clave de administrador inválida' });
        }

        // 1) Eliminar de productos_maestros.json
        let productosMaestros = readJSON(PRODUCTOS_MAESTROS_DB);
        const antesMaestros = Array.isArray(productosMaestros) ? productosMaestros.length : 0;
        if (!Array.isArray(productosMaestros)) productosMaestros = [];
        const nuevosMaestros = productosMaestros.filter(p => p.codSC !== sku);
        const eliminadosMaestros = antesMaestros - nuevosMaestros.length;
        writeJSON(PRODUCTOS_MAESTROS_DB, nuevosMaestros);

        // 2) Eliminar de productos_db.json (todas las asignaciones de clientes)
        let productosDB = readJSON(PRODUCTOS_DB);
        if (!Array.isArray(productosDB)) productosDB = [];
        const antesDB = productosDB.length;
        const nuevosDB = productosDB.filter(p => (p.codSC || p.sku) !== sku);
        const eliminadosDB = antesDB - nuevosDB.length;
        writeJSON(PRODUCTOS_DB, nuevosDB);

        if (eliminadosMaestros === 0 && eliminadosDB === 0) {
            return res.json({ ok: false, msg: 'No se encontró el SKU en la base de datos' });
        }

        res.json({ ok: true, msg: 'Producto eliminado', removidosEnClientes: eliminadosDB });
    } catch (error) {
        console.error('Error eliminar-producto-maestro:', error);
        res.status(500).json({ ok: false, msg: 'Error interno al eliminar producto' });
    }
});
...
// Endpoint para enviar OC con archivo adjunto
app.post("/api/enviar-oc-archivo", async (req, res) => {
    try {
        const { usuario, items, numeroOC, fecha, subtotal, iva, total, archivoBlob, nombreArchivo, tipoArchivo } = req.body;
        ...
        try {
            ...
            if (userIndex !== -1) {
                usersData[userIndex].ordenesCompraEnviadas = numeroOC || 1;
                fs.writeFileSync(USERS_DB, JSON.stringify(usersData, null, 2), "utf-8");
            }
        } catch (e) {
            console.warn("Error actualizando contador de OC:", e);
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                <h2 style="color: #252425; margin: 0;">✅ Tu Orden de Compra fue Adjunta Correctamente</h2>
                <p style="color: #575657; margin: 10px 0 0 0;">Nº ${numeroOC} - ${fecha}</p>
            </div>
            
            <div style="margin-bottom: 20px;">
                <p style="color: #575657; font-size: 14px;">Hola <strong>${usuario.nombre || usuario.empresa || 'Cliente'}</strong>,</p>
                <p style="color: #575657; font-size: 14px;">
                    Tu orden de compra ha sido recibida correctamente por nuestro equipo. A continuación, encontrarás los detalles de tu solicitud:
                </p>
            </div>
            
            <div style="margin-bottom: 20px;">
                <h3 style="color: #252425; border-bottom: 2px solid #BF1823; padding-bottom: 10px;">Detalle de Productos</h3>
                ${detallesHTML}
            </div>
            ...
            <div style="background-color: #e8f4f8; padding: 12px; border-radius: 8px; border-left: 4px solid #BF1823; margin-bottom: 20px;">
                <p style="margin: 0; color: #252425; font-size: 13px;">
                    <strong>📋 Próximos Pasos:</strong> Nuestro equipo revisará tu orden y se pondrá en contacto contigo en los próximos días para confirmar disponibilidad y plazos de entrega.
                </p>
            </div>
            
            <div style="background-color: #f0f0f0; padding: 12px; border-radius: 8px; margin-bottom: 20px;">
                <h4 style="margin: 0 0 8px 0; color: #252425; font-size: 13px;">📞 Datos de Contacto</h4>
                <p style="margin: 4px 0; color: #575657; font-size: 12px;"><strong>Empresa:</strong> ${usuario.empresa || 'N/A'}</p>
                <p style="margin: 4px 0; color: #575657; font-size: 12px;"><strong>Email:</strong> ${usuario.email || 'N/A'}</p>
                <p style="margin: 4px 0; color: #575657; font-size: 12px;"><strong>Teléfono:</strong> ${usuario.telefono || 'N/A'}</p>
            </div>
            
            <div style="background-color: #fff8e6; padding: 12px; border-radius: 8px; border-left: 4px solid #FFA500;">
                <p style="margin: 0; color: #252425; font-size: 12px;">
                    <strong>⚠️ Información Importante:</strong> Esta es una comunicación automática generada por la plataforma StarClutch. 
                    Para consultas, contáctanos a través de nuestro sitio web.
                </p>
            </div>
            
            <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e0e0e0; color: #575657; font-size: 11px; text-align: center;">
                <p>© 2025 STARCLUTCH S.p.A. - Todos los derechos reservados</p>
                <p style="margin: 4px 0; font-size: 10px;">Plataforma de Comercio Inteligente para la Industria Automotriz</p>
            </div>
        </div>
        `;
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="background-color: #fff3cd; padding: 12px; border-radius: 8px; border-left: 4px solid #FFC107; margin-bottom: 20px;">
                <p style="margin: 0; color: #252425; font-weight: bold;">
                    El cliente <strong>${usuario.nombre || usuario.empresa || 'Cliente'}</strong> ha enviado una nueva orden de compra
                </p>
            </div>
            
            <div style="margin-bottom: 20px;">
                <h3 style="color: #252425;">Datos del Cliente</h3>
                <p style="margin: 4px 0;"><strong>Empresa:</strong> ${usuario.empresa || 'N/A'}</p>
                <p style="margin: 4px 0;"><strong>Contacto:</strong> ${usuario.nombre || 'N/A'}</p>
                <p style="margin: 4px 0;"><strong>Email:</strong> ${usuario.email || 'N/A'}</p>
                <p style="margin: 4px 0;"><strong>Teléfono:</strong> ${usuario.telefono || 'N/A'}</p>
                <p style="margin: 4px 0;"><strong>Archivo Adjunto:</strong> ${nombreArchivo}</p>
            </div>
            ...
            <div style="background-color: #e8f4f8; padding: 12px; border-radius: 8px; border-left: 4px solid #BF1823;">
                <p style="margin: 0; color: #252425; font-size: 12px;">
                    <strong>Acción Requerida:</strong> Revisa el archivo adjunto y coordina con el cliente para confirmar disponibilidad y plazos.
                </p>
            </div>
            
            <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e0e0e0; color: #575657; font-size: 11px; text-align: center;">
                <p>© 2025 STARCLUTCH S.p.A. - Panel de Administración</p>
            </div>
        </div>
        `;
        ...
        if (archivoBlob && archivoBlob.startsWith('data:')) {
            const base64Data = archivoBlob.split(',')[1];
            archivoBuffer = Buffer.from(base64Data, 'base64');
        }
        ...
        const mailClienteOptions = {
            from: MAIL_USER,
            to: usuario.email,
            subject: `✅ Tu Orden de Compra fue Recibida - Nº ${numeroOC}`,
        ...
        mailTransport.sendMail(mailClienteOptions, (error, info) => {
            if (error) {
                console.error('Error enviando email al cliente:', error);
            } else {
                console.log('Email al cliente enviado:', info.response);
            }
        });
        ...
        mailTransport.sendMail(mailAdminOptions, (error, info) => {
            if (error) {
            ...
            console.log('Email al admin enviado:', info.response);
            res.json({ 
                ok: true, 
                msg: 'Orden de compra enviada correctamente',
                numeroOC,
        ...
        console.error('Error en /api/enviar-oc-archivo:', e);
        res.status(500).json({ ok: false, msg: 'Error procesando la orden', error: e.message });
    }
});
