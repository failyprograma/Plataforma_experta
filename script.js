...
// ========= GLOBAL: Navegación a detalle de producto (usable por todas las páginas) =========
// Provee una función global para abrir la ficha técnica desde cualquier contexto.
if (typeof window.irADetalleProducto !== 'function') {
    window.irADetalleProducto = function(valor) {
        try {
            if (!valor) return;
            // Reutiliza la lógica de CarritoGlobal si existe
            if (window.CarritoGlobal && typeof window.CarritoGlobal.irADetalleProducto === 'function') {
                window.CarritoGlobal.irADetalleProducto(valor);
                return;
            }
            ...
            const esSku = /[A-Za-z]/.test(String(valor));
            const param = esSku ? 'sku' : 'id';
            const url = `${basePath}detalleproducto.html?${param}=${encodeURIComponent(valor)}`;
            window.location.href = url;
        } catch (e) {
            console.warn('No fue posible navegar al detalle:', e);
        }
    }
}

// ========= GLOBAL: Cotizaciones pendientes en el panel de carrito =========
(function(){
    // Inserta estilos mínimos para las tarjetas de pendientes si no existen
    function ensurePendingStyles() {
    // Ya trasladamos los estilos a styles.css; sólo aseguramos z-index de modales por encima del carrito (9999)
    if (document.getElementById('sc-pending-styles')) return;
    const style = document.createElement('style');
    style.id = 'sc-pending-styles';
    style.textContent = `#pending-modal-overlay{z-index:10000;}#pending-modal{z-index:10001;}#gallery-modal-overlay{z-index:10200;}#gallery-modal{z-index:10201;}#ficha-modal-overlay{z-index:10300;}#ficha-modal{z-index:10301;}`;
    document.head.appendChild(style);
  }

    function getPendientes() {
        try {
            return JSON.parse(localStorage.getItem('starclutch_cotizaciones_pendientes') || '[]');
        } catch { return []; }
    }
    ...
    function createContainerIfNeeded() {
        ...
        if (!body) return null;
        let cont = body.querySelector('#cart-pending-container');
        if (!cont) {
            cont = document.createElement('div');
            cont.id = 'cart-pending-container';
            // No agregamos título para coincidir con carrito.html
            body.appendChild(cont);
        }
        return cont;
    }

    function renderPendientesEnCartPanel() {
        ...
        pendientes.forEach(p => {
            ...
            closeBtn.onclick = (e) => {
                e.stopPropagation();
                const arr = getPendientes().filter(x => String(x.id) !== String(p.id));
                localStorage.setItem('starclutch_cotizaciones_pendientes', JSON.stringify(arr));
                renderPendientesEnCartPanel();
            };
            ...
            // Items con nombre y cantidad
            const itemsDiv = document.createElement('div');
            itemsDiv.className = 'cart-pending-items';
            ...
            actions.className = 'cart-pending-actions';
            const subir = document.createElement('button');
            subir.className = 'btn-primary';
            subir.textContent = 'Subir OC';
            subir.onclick = () => { subirOrdenDeCompraGlobal(p); };
            const detalles = document.createElement('button');
            detalles.className = 'btn-secondary';
            detalles.textContent = 'Ver detalles';
            detalles.onclick = () => { verDetallesPendienteGlobal(p); };
            actions.appendChild(subir);
            actions.appendChild(detalles);
    ...
    // Crea modales si no existen en la página
    function ensurePendingModals() {
        if (!document.getElementById('pending-modal-overlay')) {
        ...
        if (!document.getElementById('pending-modal')) {
            ...
            modal.innerHTML = `
                <div class="oc-modal-header">
                    ...
                    <button class="oc-modal-close" onclick="cerrarModalPendienteGlobal()">✕</button>
                </div>
                ...
                <div class="oc-modal-footer">
                    ...
                    <div class="oc-modal-actions">
                        <button class="btn-secondary" onclick="cerrarModalPendienteGlobal()">Cerrar</button>
                        <button class="btn-primary" onclick="subirOrdenDeCompraDesdeModalGlobal()">Subir OC</button>
                    </div>
                </div>
            `;
        ...
        if (!document.getElementById('gallery-modal-overlay')) {
            ...
            overlay.className = 'oc-modal-overlay';
            overlay.style.cssText = 'z-index: 10200;';
            overlay.onclick = cerrarGaleriaImagenesGlobal;
            document.body.appendChild(overlay);
        }
        if (!document.getElementById('gallery-modal')) {
            const modal = document.createElement('div');
            modal.id = 'gallery-modal';
            modal.className = 'pending-gallery-modal';
            modal.style.cssText = 'z-index: 10201;';
            modal.innerHTML = `
                <div class="gallery-header">
                    <h2>Galería</h2>
                    <button class="gallery-close-btn" onclick="cerrarGaleriaImagenesGlobal()">✕</button>
                </div>
                <div class="gallery-container">
                    <button class="gallery-nav-btn gallery-prev" onclick="galeriaPrevGlobal()">‹</button>
                    <div class="gallery-image-wrapper">
                        <img id="gallery-main-image" src="" alt="Producto" class="gallery-main-image">
                    </div>
                    <button class="gallery-nav-btn gallery-next" onclick="galeriaNextGlobal()">›</button>
                </div>
    ...
    function cerrarModalPendienteGlobal(){
        const ov = document.getElementById('pending-modal-overlay');
        const md = document.getElementById('pending-modal');
    ...
    function ensureFichaTecnicaModal() {
        if (!document.getElementById('ficha-modal-overlay')) {
            const overlay = document.createElement('div');
            overlay.id = 'ficha-modal-overlay';
            overlay.className = 'oc-modal-overlay';
            overlay.style.cssText = 'z-index: 10300;';
            overlay.onclick = cerrarFichaTecnicaGlobal;
            document.body.appendChild(overlay);
        }
        if (!document.getElementById('ficha-modal')) {
            ...
            modal.className = 'oc-modal';
            modal.style.cssText = 'z-index: 10301; width: 700px;';
            modal.innerHTML = `
                <div class="oc-modal-header">
                    <h2 id="ficha-modal-title">Ficha Técnica del Producto</h2>
                    <button class="oc-modal-close" onclick="cerrarFichaTecnicaGlobal()">✕</button>
                </div>
                <div class="oc-modal-content">
                ...
                <div class="oc-modal-footer">
                    <div class="oc-modal-actions">
                        <button class="btn-secondary" onclick="cerrarFichaTecnicaGlobal()">Cerrar</button>
                    </div>
                </div>`;
            document.body.appendChild(modal);
        }
    ...
    function abrirFichaTecnicaGlobal(sku) {
        ensureFichaTecnicaModal();
        cargarFichaTecnica(sku);
    ...
    function cerrarFichaTecnicaGlobal() {
        const overlay = document.getElementById('ficha-modal-overlay');
        const modal = document.getElementById('ficha-modal');
        if (overlay) {
    ...
    async function cargarFichaTecnica(sku) {
        const content = document.getElementById('ficha-content');
        if (!content) return;
        
        content.innerHTML = '<p style="text-align:center;padding:20px;color:#575657;">Cargando...</p>';
        
        try {
            const productos = await cargarProductosParaModalGlobal();
            const producto = productos.find(p => p.codSC === sku);
            
            if (!producto) {
                content.innerHTML = '<p style="text-align:center;padding:20px;color:#BF1823;">Producto no encontrado</p>';
                return;
            }
            ...
            // Ficha técnica
            if (producto.fichaTecnica && producto.fichaTecnica.trim()) {
                html += `<textarea class="ficha-textarea" readonly>${producto.fichaTecnica}</textarea>`;
            } else {
                html += `<textarea class="ficha-textarea" readonly placeholder="No hay información técnica disponible"></textarea>`;
            }
            ...
            // Referencia cruzada
            if (producto.referenciaCruzada && producto.referenciaCruzada.trim()) {
                html += `<textarea class="ficha-textarea" readonly>${producto.referenciaCruzada}</textarea>`;
            } else {
                html += `<textarea class="ficha-textarea" readonly placeholder="No hay referencias cruzadas disponibles"></textarea>`;
            }
            ...
            // Códigos OEM
            if (producto.oem && producto.oem.trim()) {
                html += `<textarea class="ficha-textarea" readonly>${producto.oem}</textarea>`;
            } else {
                html += `<textarea class="ficha-textarea" readonly placeholder="No hay códigos OEM disponibles"></textarea>`;
            }
            ...
            content.innerHTML = '<p style="text-align:center;padding:20px;color:#BF1823;">Error al cargar la información</p>';
        }
    }
    // ======== FIN MODAL DE FICHA TÉCNICA ========

    // Cargar productos para modal (path relativo según ubicación)
    async function cargarProductosParaModalGlobal(){
        const path = window.location.pathname;
        let jsonPath = '/datosproductos/productos_db.json';
        if (!path.includes('/Plataforma_experta-master/')) {
            // Fallbacks según carpetas
            if (path.includes('/mis flotas/') || path.includes('/mis%20flotas/')) jsonPath = '../datosproductos/productos_db.json';
            else jsonPath = '/datosproductos/productos_db.json';
        }
        try {
    ...
    async function verDetallesPendienteGlobal(pendiente){
        ensurePendingStyles();
        ensurePendingModals();
        const num = document.getElementById('pending-numero-cot');
        const fecha = document.getElementById('pending-fecha');
        const estado = document.getElementById('pending-estado');
        const items = document.getElementById('pending-items-lista');
        const productosDB = await cargarProductosParaModalGlobal();
        if (num) num.textContent = pendiente.numeroCotizacion || '-';
        if (fecha) fecha.textContent = pendiente.fecha || '-';
        if (estado) estado.textContent = 'Pendiente';
        if (items) items.innerHTML = '';
        (pendiente.items || []).forEach(item => {
            const prod = productosDB.find(p => p.codSC === item.sku || p.id === item.id) || {};
            const images = Array.isArray(prod.imagenes) && prod.imagenes.length ? prod.imagenes : ['../img/productos.svg'];
            
            // Guardar imágenes en variable global con key del SKU
            if (!window.galeriaImagenesStorage) window.galeriaImagenesStorage = {};
            window.galeriaImagenesStorage[item.sku] = images;
            ...
            div.innerHTML = `
                <div class="pending-item-img-container">
                    <img class="pending-item-img" src="${images[0]}" alt="${item.nombre}" onclick="event.stopPropagation(); abrirGaleriaImagenesDesdeItem('${item.sku}')" style="cursor:pointer;" onerror="this.src='../img/productos.svg'">
                    ${images.length>1?`<div class='pending-item-gallery-indicator'>${images.length} imágenes</div>`:''}
                </div>
                <div class="pending-item-info">
                    ...
                    <div class="pending-item-detalles">
                        <div class="pending-item-detalle-row"><strong>Cantidad:</strong> ${item.cantidad}</div>
                        <div class="pending-item-detalle-row"><strong>Precio Unit.:</strong> ${CarritoGlobal.formatearPrecio(item.precio)}</div>
                        <div class="pending-item-detalle-row"><strong>Subtotal:</strong> ${CarritoGlobal.formatearPrecio(item.precio * item.cantidad)}</div>
                    </div>
                </div>
                <div class="pending-item-actions">
                    <button class="btn-secondary pending-item-btn-ficha" onclick="event.stopPropagation(); abrirFichaTecnicaGlobal('${item.sku}')">
            ...
            // Agregar evento click a la tarjeta para navegar a detalleproducto.html
            div.addEventListener('click', () => {
                // Guardar producto completo en sessionStorage
                ...
                // Guardar todos los productos de la orden para el carrusel
                const productosOrden = (pendiente.items || []).map(i => {
                    const p = productosDB.find(db => db.codSC === i.sku || db.id === i.id) || {};
                    return p;
                }).filter(p => p.codSC);
                
                if (productosOrden.length > 0) {
                    sessionStorage.setItem('productosCarrusel', JSON.stringify(productosOrden));
                    sessionStorage.setItem('origenCarrusel', 'orden-pendiente');
                }
                ...
                window.location.href = basePath + 'detalleproducto.html?sku=' + encodeURIComponent(item.sku);
            });
        ...
        // Guardar y cerrar el carrito
        const cartPanel = document.getElementById('cart-panel');
        const cartOverlay = document.getElementById('cart-overlay');
        
        // Guardar estado antes de cerrar
        window.pendingModalCartState = {
            panelOpen: cartPanel ? cartPanel.classList.contains('open') : false,
            overlayVisible: cartOverlay ? cartOverlay.classList.contains('visible') : false
        };
        
        // Cerrar carrito usando las clases CSS (como lo hace cerrarCarrito() en las páginas)
        if (cartPanel) {
        ...
        // Abrir modal con pequeño delay para asegurar que el carrito se cerró
        setTimeout(() => {
    ...
    function subirOrdenDeCompraDesdeModalGlobal(){
        if (window.pendienteActual) {
            const pendienteCopia = { ...window.pendienteActual };
            cerrarModalPendienteGlobal();
            subirOrdenDeCompraGlobal(pendienteCopia);
        }
    }



    async function subirOrdenDeCompraGlobal(pendiente) {
      console.log('subirOrdenDeCompraGlobal - pendiente recibido:', pendiente);
          ...
          reader.onload = async (event) => {
            ...
            try {
              ...
              if (respuesta.ok) {
                ...
                // Eliminar la orden pendiente (no solo cambiar estado)
                pendientes = pendientes.filter(p => p.id !== pendiente.id);
                localStorage.setItem('starclutch_cotizaciones_pendientes', JSON.stringify(pendientes));

                // Actualizar la vista del carrito
                if (typeof renderPendientesEnCartPanel === 'function') {
                  renderPendientesEnCartPanel();
                }
    ...
    function abrirGaleriaImagenesDesdeItem(sku) {
        console.log('abrirGaleriaImagenesDesdeItem llamada con SKU:', sku);
        if (!window.galeriaImagenesStorage) {
            console.error('No hay almacenamiento de imágenes');
            return;
        }
        const imagenes = window.galeriaImagenesStorage[sku];
        console.log('Imágenes encontradas para SKU:', sku, imagenes);
        if (!imagenes) {
            console.error('No se encontraron imágenes para SKU:', sku);
            return;
        }
    ...
    function abrirGaleriaImagenesGlobal(sku, imagenes){
        ...
        if (!Array.isArray(imagenes)) {
            console.error('imagenes no es un array:', imagenes);
            if (typeof imagenes === 'string') {
                try {
                    console.log('Intentando parsear string:', imagenes);
                    galeriaImagenes = JSON.parse(imagenes);
                } catch (e) {
                    console.error('Error parseando:', e);
                    galeriaImagenes = [imagenes]; // al menos la imagen como string
                }
            } else {
        ...
        console.log('galeriaImagenes final:', galeriaImagenes);
        
        if (!Array.isArray(galeriaImagenes) || galeriaImagenes.length === 0) {
            console.error('galeriaImagenes sigue siendo inválida:', galeriaImagenes);
            alert('No hay imágenes para mostrar');
            return;
        }
        
        galeriaIndiceActual = 0;
        const ov = document.getElementById('gallery-modal-overlay');
        const md = document.getElementById('gallery-modal');
        
        console.log('Elementos encontrados:', { ov: !!ov, md: !!md });
        
        if (ov) {
            ov.classList.add('active');
            console.log('Overlay abierto');
        }
        if (md) {
            md.classList.add('active');
            console.log('Modal abierto, mostrando imagen 0');
        }
        
        mostrarImagenGaleriaGlobal(0);
    }
    function cerrarGaleriaImagenesGlobal(){
        const ov = document.getElementById('gallery-modal-overlay');
        const md = document.getElementById('gallery-modal');
        if (ov) {
    ...
    function galeriaPrevGlobal(){
        galeriaIndiceActual = (galeriaIndiceActual - 1 + galeriaImagenes.length) % galeriaImagenes.length;
        mostrarImagenGaleriaGlobal(galeriaIndiceActual);
    }
    function galeriaNextGlobal(){
        galeriaIndiceActual = (galeriaIndiceActual + 1) % galeriaImagenes.length;
        mostrarImagenGaleriaGlobal(galeriaIndiceActual);
    }
    function mostrarImagenGaleriaGlobal(idx){
        const main = document.getElementById('gallery-main-image');
        const thumbs = document.getElementById('gallery-thumbnails');
        if (!main || !thumbs) {
            console.error('Elementos de galería no encontrados:', { main, thumbs });
            return;
        }
        if (!Array.isArray(galeriaImagenes) || galeriaImagenes.length === 0) {
            console.error('galeriaImagenes no es un array válido:', galeriaImagenes);
            return;
        }
        const src = galeriaImagenes[idx] || '../img/productos.svg';
        console.log('Mostrando imagen:', idx, 'src:', src, 'galeriaImagenes:', galeriaImagenes);
        
        // Corregir rutas relativas si es necesario
        let finalSrc = src;
        if (src.startsWith('../')) {
            // Si estamos en una página dentro de una carpeta, ajustar la ruta
            const currentPath = window.location.pathname;
            if (currentPath.includes('/mis flotas/') || currentPath.includes('/mis%20flotas/')) {
                finalSrc = src; // está bien, está subiendo un nivel
            } else if (!currentPath.includes('/Plataforma_experta-master/')) {
                // En otras páginas root-level, convertir a ruta absoluta
                finalSrc = '/datosproductos/productos_db.json'.replace('productos_db.json', '') + src.substring(3);
            }
        }
        ...
        main.onerror = () => {
            ...
            main.src = '../img/productos.svg';
        };
        
        thumbs.innerHTML = '';
        galeriaImagenes.forEach((im, i) => {
            const t = document.createElement('img');
            t.src = im;
            t.style.width = '60px';
            t.style.height = '60px';
            t.style.objectFit = 'cover';
            t.style.border = i===idx ? '2px solid #BF1823' : '1px solid #e5e7eb';
            t.style.borderRadius = '8px';
            t.style.cursor = 'pointer';
            t.onclick = () => { galeriaIndiceActual = i; mostrarImagenGaleriaGlobal(i); };
    ...
    function shouldRenderHere(){
        const path = window.location.pathname;
        if (path.includes('/administrador/')) return false;
        if (path.includes('/login') || path.includes('/index.html') && path.includes('/estado de la cuenta/')) return true; // allow most pages
        return true;
    }

    document.addEventListener('DOMContentLoaded', () => {
        if (!shouldRenderHere()) return;
        try { renderPendientesEnCartPanel(); } catch(e){ console.warn('No se pudo renderizar pendientes:', e); }
    });
})();
/* =========================================================
...
function renderizarFlotaEnGridMarcasModelos() {
    ...
    setTimeout(() => {
        if (typeof initCarruselGruposFlotas === 'function') {
            initCarruselGruposFlotas();
        }
    }, 100);
}

// ================================
// FUNCIÓN PARA INICIALIZAR CARRUSEL DE GRUPOS (REUTILIZABLE)
// ================================
function initCarruselGruposFlotas() {
    ...
    // Crear nuevos botones clonados para eliminar listeners viejos
    ...
    // Ahora obtener referencias a los botones nuevos (sin listeners viejos)
    ...
    function updateSlidePosition() {
        ...
        // Deshabilitar botones en los extremos, o ambos si solo hay 1 página
        if (numIndicators <= 1) {
            // Si hay una sola página, deshabilitar ambos botones
            if (prevBtn) prevBtn.disabled = true;
            if (nextBtn) nextBtn.disabled = true;
        } else {
            // Si hay múltiples páginas, deshabilitar según posición
            if (prevBtn) {
...
async function cargarFlotasDelClienteAdmin(userId) {
    ...
    try {
        ...
        if (flotas.length === 0) {
            ...
            // Limpiar ID de flota seleccionada
            adminIdFlotaSeleccionada = null;
        } else {
            flotas.forEach((f, index) => {
                const opt = document.createElement('option');
                opt.value = f.id;
                opt.textContent = f.nombre;
                // Seleccionar la primera flota automáticamente
                if (index === 0) {
                    opt.selected = true;
                }
                selectFlota.appendChild(opt);
            ...
            // Establecer ID de la primera flota seleccionada
            adminIdFlotaSeleccionada = flotas[0].id;
            
            // Cargar vehículos de la primera flota automáticamente
            if (flotas.length > 0) {
                await cargarVehiculosFlotaAdmin(flotas[0].id);
        ...
        selectFlota.innerHTML = '<option>Error al cargar</option>';
        adminIdFlotaSeleccionada = null;
    }
}
...
async function adminGuardarEdicionVehiculo() {
    ...
    // Sin validación - Admin puede dejar campos vacíos
        ...
        tr.innerHTML = `
            ...
            <td style="text-align:center;">
                ${(p.fichaTecnica || p.referenciaCruzada || p.oem)
                    ? `<button class="btn-text-ficha" onclick="verFichaTecnicaAdmin(this)"><img src="/img/fichatecnica.svg" alt="Ficha Técnica"> Ver</button>`
    ...
    if (modal) {
        ...
        if (tablaUpload) {
            ...
            if (tbody) {
                ...
                // Tabla vacía inicialmente - usar botón "+ Agregar otro producto" para agregar filas
            }
        }
    }
}
...
async function cargarProductosAlCliente() {
    ...
    try {
        const resCliente = await fetch(`/api/obtener-productos?userId=${adminSelectedClientId}`);
        const dataCliente = await resCliente.json();
        
        if (resCliente.ok) {
            const productosCliente = dataCliente.productos || [];
            const duplicadosEncontrados = [];
            
            productosMetadata.forEach(prod => {
                const yaExiste = productosCliente.some(p => p.codSC === prod.codSC);
                if (yaExiste) {
                    duplicadosEncontrados.push(prod.codSC);
                }
            });
            
            if (duplicadosEncontrados.length > 0) {
                let mensaje = '⚠️ PRODUCTOS DUPLICADOS DETECTADOS\n\n';
                mensaje += 'Los siguientes SKUs ya existen en la lista del cliente:\n\n';
                duplicadosEncontrados.forEach(sku => {
                    mensaje += `• ${sku}\n`;
                });
                mensaje += '\nPor favor elimina las filas duplicadas antes de continuar.';
                
                alert(mensaje);
                
                // Resaltar las filas duplicadas
                const filas = document.querySelectorAll('#tabla-productos-upload tbody tr');
                filas.forEach((row, index) => {
                    const codSC = row.querySelector('.input-cod-sc').value.trim();
                    if (duplicadosEncontrados.includes(codSC)) {
                        row.style.backgroundColor = '#ffe6e6';
...
// =========================================================
// 3. BÚSQUEDA DE PRODUCTOS MAESTROS (NUEVA FUNCIONALIDAD)
// =========================================================

// Variables para búsqueda y selección
let timeoutSugerencias = null;
let productosSeleccionadosBusqueda = new Map(); // SKU -> producto completo

// Cargar sugerencias de SKU mientras se escribe
async function cargarSugerenciasSKU(query) {
    const dropdown = document.getElementById('sugerencias-sku-dropdown');
    ...
    // Si la query es muy corta, ocultar dropdown
    ...
    timeoutSugerencias = setTimeout(async () => {
        try {
            // Primero, obtener los SKUs que ya tiene el cliente
            let skusDuplicados = new Set();
            if (adminSelectedClientId) {
                try {
                    const resCliente = await fetch(`/api/obtener-productos?userId=${adminSelectedClientId}`);
                    if (resCliente.ok) {
                        const dataCliente = await resCliente.json();
                        console.log('Datos brutos del cliente:', dataCliente);
                        
                        // El endpoint retorna un array directamente, no un objeto
                        let productosCliente = [];
                        if (Array.isArray(dataCliente)) {
                            productosCliente = dataCliente;
                        } else if (dataCliente && dataCliente.productos) {
                            productosCliente = dataCliente.productos;
                        }
                        
                        console.log('Productos del cliente parseados:', productosCliente.length);
                        
                        productosCliente.forEach(p => {
                            if (p.codSC) {
                                skusDuplicados.add(p.codSC.trim().toUpperCase());
                            }
                        });
                    }
                } catch (error) {
            ...
            console.log('SKUs duplicados encontrados:', Array.from(skusDuplicados));
            
            const res = await fetch(`/api/sugerencias-sku?query=${encodeURIComponent(query)}`);
            const data = await res.json();
            
            if (res.ok && data.sugerencias && data.sugerencias.length > 0) {
                // Construir el dropdown con checkboxes
                dropdown.innerHTML = data.sugerencias.map(sug => {
                    const isChecked = productosSeleccionadosBusqueda.has(sug.sku);
                    const isDuplicate = skusDuplicados.has(sug.sku.trim().toUpperCase());
                    
                    return `
                    <div class="sugerencia-item" 
                         style="padding: 12px 16px; cursor: ${isDuplicate ? 'not-allowed' : 'pointer'}; border-bottom: 1px solid #e0e0e0; transition: background 0.2s; background: ${isDuplicate ? '#ffebee' : (isChecked ? '#e8f5e9' : 'white')}; opacity: ${isDuplicate ? '0.6' : '1'};"
                         onmouseover="if(!this.dataset.isDuplicate) this.style.background='${isChecked ? '#e8f5e9' : '#e3f2fd'}'"
                         onmouseout="this.style.background='${isDuplicate ? '#ffebee' : (isChecked ? '#e8f5e9' : 'white')}'"
                         data-is-duplicate="${isDuplicate}"
                         onclick="${isDuplicate ? '' : `toggleSeleccionProducto('${sug.sku}', ${JSON.stringify(sug).replace(/"/g, '&quot;')})`}">
                        <div style="display: flex; gap: 12px; align-items: center; user-select: none;">
                            ${isDuplicate ? 
                                `<div style="width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; background: #ffcdd2; border-radius: 3px; font-size: 12px; color: #c62828; font-weight: bold;">✓</div>` 
                                : 
                                `<input type="checkbox" 
                                        id="check-${sug.sku}" 
                                        ${isChecked ? 'checked' : ''}
                                        style="width: 18px; height: 18px; cursor: pointer; accent-color: #4caf50; pointer-events: none;"
                                        readonly>`
                            }
                            <div style="flex: 1;">
                                <div style="font-weight: 600; color: ${isDuplicate ? '#c62828' : '#2c5aa0'}; font-size: 14px; margin-bottom: 4px;">
                                    ${sug.sku} ${isDuplicate ? '<span style="font-size: 11px; color: #c62828;">(YA EXISTE)</span>' : ''}
                                </div>
                                <div style="font-size: 12px; color: ${isDuplicate ? '#999' : '#666'};">
                                    ${sug.repuesto} - <span style="color: #BF1823;">${sug.marca}</span>
                                </div>
                            </div>
                            <div style="background: ${isDuplicate ? '#ffcdd2' : '#e3f2fd'}; padding: 4px 8px; border-radius: 4px; font-size: 11px; color: ${isDuplicate ? '#c62828' : '#2c5aa0'};">
                                ${sug.linea}
                            </div>
                        </div>
                    </div>
...
// Toggle selección de producto con checkbox
async function toggleSeleccionProducto(sku, productoDataStr) {
    // Parsear el objeto del producto
    let producto;
    try {
        producto = typeof productoDataStr === 'string' 
            ? JSON.parse(productoDataStr.replace(/&quot;/g, '"'))
            : productoDataStr;
    } catch (e) {
        console.error('Error parsing producto:', e);
        return;
    }
    
    const checkbox = document.getElementById(`check-${sku}`);
    if (!checkbox) return;
    
    const willBeChecked = !checkbox.checked;
    
    // Verificar si el cliente ya tiene este producto (solo si va a marcarse)
    if (willBeChecked && adminSelectedClientId) {
        try {
            const resCliente = await fetch(`/api/obtener-productos?userId=${adminSelectedClientId}`);
            if (resCliente.ok) {
                const dataCliente = await resCliente.json();
                if (dataCliente && dataCliente.productos) {
                    const yaExiste = dataCliente.productos.some(p => p.codSC === sku);
                    if (yaExiste) {
                        alert(`⚠️ PRODUCTO DUPLICADO\n\nEl cliente ya tiene el producto "${sku}" en su lista.\n\nNo se puede agregar porque ya existe.`);
                        return;
                    }
                }
            }
    ...
    // Actualizar el mapa de seleccionados
    if (checkbox.checked) {
        productosSeleccionadosBusqueda.set(sku, producto);
    } else {
        productosSeleccionadosBusqueda.delete(sku);
    }
    ...
    const sugerencionItem = checkbox.closest('.sugerencia-item');
    if (sugerencionItem) {
        sugerencionItem.style.background = checkbox.checked ? '#e8f5e9' : 'white';
    }
}
...
function actualizarProductosSeleccionados() {
    ...
    if (productosSeleccionadosBusqueda.size > 0) {
        container.style.display = 'block';
        count.textContent = productosSeleccionadosBusqueda.size;
        
        const items = Array.from(productosSeleccionadosBusqueda.values())
            .map(p => `• ${p.sku} - ${p.repuesto}`)
            .join('<br>');
...
async function agregarProductosSeleccionados() {
    ...
    if (!adminSelectedClientId) {
        alert('⚠️ Por favor selecciona un cliente antes de agregar productos');
        return;
    }
    
    // Obtener SKUs del servidor (datos guardados del cliente)
    let skusEnServidor = new Set();
    
    try {
        const resCliente = await fetch(`/api/obtener-productos?userId=${adminSelectedClientId}`);
        console.log('Response status:', resCliente.status);
        if (resCliente.ok) {
            const dataCliente = await resCliente.json();
            ...
            console.log('Productos del cliente encontrados:', productosCliente.length);
            productosCliente.forEach(p => {
                if (p.codSC) {
                    const skuLimpio = p.codSC.trim().toUpperCase();
                    skusEnServidor.add(skuLimpio);
                    console.log('SKU agregado desde servidor:', skuLimpio);
                }
            });
        }
    } catch (error) {
    ...
    // También verificar la tabla DOM para productos no guardados aún
    const tabla = document.getElementById('tabla-productos-upload');
    const tbody = tabla.querySelector('tbody');
    const filasExistentes = tbody.querySelectorAll('tr');
    const skusEnTabla = new Set();
    
    filasExistentes.forEach(fila => {
        const inputSKU = fila.querySelector('.input-cod-sc');
        if (inputSKU && inputSKU.value.trim()) {
            const skuLimpio = inputSKU.value.trim().toUpperCase();
            skusEnTabla.add(skuLimpio);
            console.log('SKU agregado desde tabla DOM:', skuLimpio);
        }
    });
    
    // Combinar ambos sets (servidor + tabla)
    const skusExistentes = new Set([...skusEnServidor, ...skusEnTabla]);
    
    console.log('=== DEBUG VALIDACIÓN ===');
    console.log('Total SKUs existentes:', skusExistentes.size);
    console.log('SKUs existentes:', Array.from(skusExistentes));
    console.log('SKUs seleccionados para agregar:', Array.from(productosSeleccionadosBusqueda.keys()));
    
    // Primero, verificar cuáles ya existen
    const productosAVerificar = Array.from(productosSeleccionadosBusqueda.entries());
    const duplicados = [];
    const nuevos = [];
    
    for (const [sku, producto] of productosAVerificar) {
        const skuLimpio = sku.trim().toUpperCase();
        console.log(`Comparando ${skuLimpio} - ¿Existe?: ${skusExistentes.has(skuLimpio)}`);
        if (skusExistentes.has(skuLimpio)) {
            duplicados.push({ sku, repuesto: producto.repuesto });
        } else {
            nuevos.push([sku, producto]);
        }
    }
    ...
    if (duplicados.length > 0) {
        let mensaje = `⚠️ ${duplicados.length} PRODUCTO(S) YA EXISTEN EN LA LISTA\n\n`;
        mensaje += 'Los siguientes productos NO se agregarán (ya están en la lista):\n\n';
        duplicados.forEach(dup => {
            mensaje += `• ${dup.sku} - ${dup.repuesto}\n`;
        });
        
        if (nuevos.length > 0) {
            mensaje += `\n✅ Se agregarán ${nuevos.length} NUEVO(S) PRODUCTO(S):\n\n`;
            nuevos.forEach(([sku, prod]) => {
                mensaje += `• ${sku} - ${prod.repuesto}\n`;
            });
        } else {
            ...
            // Limpiar selección
            productosSeleccionadosBusqueda.clear();
            actualizarProductosSeleccionados();
            ...
            // Limpiar input
            const input = document.getElementById('buscar-sku-rapido');
            input.value = '';
            
            return;
        }
        
        if (!confirm(mensaje + '\n\n¿Deseas continuar agregando solo los nuevos?')) {
            return;
        }
    }
    
    // Si no hay nuevos productos, detener
    if (nuevos.length === 0) {
        alert('⛔ Todos los productos seleccionados ya existen en la lista del cliente.');
    ...
    for (const [sku, productoBasico] of nuevos) {
        try {
            // Buscar información completa del producto
            const res = await fetch(`/api/buscar-producto-maestro?sku=${encodeURIComponent(sku)}`);
            if (!res.ok) continue;
            ...
            tr.innerHTML = `
                <td><input type="text" class="sc-input-small input-cod-cli" value="${producto.codCliente || ''}" placeholder="Opcional"></td>
                <td><input list="lista-repuestos" class="sc-input-small input-repuesto" value="${producto.repuesto}" placeholder="Escribe..." oninput="detectarLinea(this)"></td>
                <td><input list="lista-marcas" class="sc-input-small input-marca" value="${producto.marca}" placeholder="Marca..."></td>
                <td><input type="text" class="sc-input-small input-linea" readonly value="${producto.linea}" placeholder="Auto"></td>
                <td><input type="text" class="sc-input-small input-cod-sc" value="${producto.codSC}" placeholder="Requerido" onblur="verificarSKUExistente(this)"></td>
                ...
                <td><input type="number" class="sc-input-small input-stock" value="${producto.stock || 0}" placeholder="0" min="0" step="1"></td>
                <td>
                    <div class="upload-wrapper">
                        ...
                        <div class="file-list-container" style="display:flex; gap:5px; margin-top:5px; flex-wrap:wrap;">
                            ${(producto.imagenes || []).map(img => 
                                `<div class="file-preview-item" style="position:relative;">
                                    <img src="${img}" style="width:40px; height:40px; object-fit:cover; border-radius:4px; cursor:pointer;" onclick="verImagenPreview('${img}', '${img.split('/').pop()}')">
                                    <span style="position:absolute; top:-5px; right:-5px; background:#BF1823; color:white; border-radius:50%; width:16px; height:16px; display:flex; align-items:center; justify-content:center; font-size:10px; cursor:pointer;" onclick="this.parentElement.remove()">&times;</span>
                                </div>`
                            ).join('')}
                        </div>
                    </div>
                ...
                <td style="text-align:center;">
                    <button class="btn-ficha-tecnica ${(producto.fichaTecnica || producto.referenciaCruzada || producto.oem) ? 'has-data' : ''}" onclick="abrirModalFichaTecnicaFila(this)">
                        <img src="../img/fichatecnica.svg" alt="Ficha Técnica">
            ...
            if (producto.fichaTecnica || producto.referenciaCruzada || producto.oem) {
                fichasTecnicasPorFila[indexNuevo] = {
            ...
            console.error(`Error al agregar producto ${sku}:`, error);
        }
    }
    ...
    let resumenFinal = `✅ OPERACIÓN COMPLETADA\n\n`;
    resumenFinal += `Productos agregados a la tabla: ${productosAgregados}\n`;
    if (duplicados.length > 0) {
        resumenFinal += `Productos duplicados ignorados: ${duplicados.length}\n`;
    }
    resumenFinal += `\nAhora define el precio y descuento para cada producto.`;
...
// Verificar si SKU existe cuando se pierde el foco del campo
async function verificarSKUExistente(input) {
    const sku = input.value.trim();
    if (!sku) return;
    
    const row = input.closest('tr');
    const inputRepuesto = row.querySelector('.input-repuesto');
    
    // Solo verificar si el campo de repuesto está vacío
    if (inputRepuesto.value.trim()) return;
    ...
    try {
        ...
        if (res.ok && data.encontrado) {
            ...
            input.title = 'Este SKU existe en la base de datos.';
        } else {
...
function agregarFila() {
    ...
    tr.innerHTML = `
        ...
        <td><input type="text" class="sc-input-small input-cod-sc" placeholder="Requerido" onblur="verificarSKUExistente(this)"></td>
        <td><input type="text" class="sc-input-small input-precio" placeholder="0" min="0" oninput="formatearPrecioInput(this)"></td>
        <td><input type="number" class="sc-input-small input-descuento" placeholder="0" step="0.01" min="0" max="100"></td>
        ...
        <td style="text-align:center;">
            <button class="btn-ficha-tecnica" onclick="abrirModalFichaTecnicaFila(this)">
                <img src="/img/fichatecnica.svg" alt="Ficha Técnica">
                Agregar
            </button>
        </td>
...
// (Eliminada duplicación de verFotos - se usa la versión definida antes en el archivo)
...
const CarritoGlobal = {
    ...
    // Contar productos (incluye órdenes pendientes como 1 cada una)
    contarProductos() {
        const carrito = this.obtenerLocal();
        const productosEnCarrito = carrito.items.reduce((total, item) => total + (item.cantidad || 0), 0);
        let pendientesCount = 0;
        try {
            const pendientes = JSON.parse(localStorage.getItem('starclutch_cotizaciones_pendientes') || '[]');
            // Contar sólo pendientes no enviados (o sin estado)
            pendientesCount = Array.isArray(pendientes)
                ? pendientes.filter(p => !p.estado || p.estado !== 'enviado').length
                : 0;
        } catch (e) {
            pendientesCount = 0;
        }
        return productosEnCarrito + pendientesCount;
    },
    ...
    actualizarUI() {
        ...
        // Cálculos detallados: productos + pendientes
        const productosEnCarrito = Array.isArray(carrito.items)
        ...
        const totalProductos = productosEnCarrito + pendientesCount;

        // Actualizar badge del header (existe en todas las páginas con carrito)
        const cartBadgeCount = document.getElementById('cart-badge-count');
        if (cartBadgeCount) {
            cartBadgeCount.textContent = totalProductos;
            // Mostrar/ocultar badge si hay productos o pendientes
            cartBadgeCount.style.display = totalProductos > 0 ? 'flex' : 'none';

            // Si hay al menos 1 pendiente y (al menos 1 producto o más de 1 pendiente), agregar stroke naranja
            const showOCStroke = (pendientesCount > 1) || (pendientesCount >= 1 && productosEnCarrito >= 1);
            if (showOCStroke) {
                cartBadgeCount.classList.add('has-oc');
            } else {
                cartBadgeCount.classList.remove('has-oc');
            }
        }
